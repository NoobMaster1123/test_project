...
class IndexView(FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        SyncDB.sync_currencies(list(set([currency_code[1] for currency_code in country_currencies])))
    ...
    def form_valid(self, form):
        ...
        if not ExchangeRate.objects.filter(date=base_date).exists():
            currency_rates = Scraper.parse_currency_rates(base_date,
                                                          base_date+datetime.timedelta(days=1), currency_codes)
            SyncDB.sync_exchange_rates(currency_rates)
