...
class CurrencyPlot:
    ...
    def plot_relative_change(self, ax):
        ...
        for currency_code in self.currency_codes:
            currency_data = self.df[self.df['currency_code'] == currency_code]
            ax.plot(currency_data['date'], currency_data['relative_change'], linestyle='dashed', label=f'{currency_code} Change')
